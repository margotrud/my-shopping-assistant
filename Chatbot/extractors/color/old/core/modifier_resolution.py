#Chatbot/extractors/color/core/modifier_resolution.py
...
from Chatbot.extractors.color.old.core.matcher import fuzzy_match_modifier
...
def _override_if_tone_suffix(word: str, known_tones: Optional[Set[str]]) -> Optional[str]:
    """
        Attempts to override a word ending in 'y' by checking if its base form
        (with the 'y' stripped) exists in the known tones set.

        This is useful for handling suffix-based variants like 'peachy' → 'peach'.

        Args:
            word (str): The word to evaluate (e.g., 'peachy').
            known_tones (Optional[Set[str]]): Set of valid tone names (e.g., {'peach', 'rose'}).

        Returns:
            Optional[str]: The base tone (e.g., 'peach') if found; otherwise None.
        """
